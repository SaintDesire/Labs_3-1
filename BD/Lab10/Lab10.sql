...
DECLARE
  ...
  -- Объявление вложенного типа записи
...
BEGIN
  -- Присвоение значений полям типа записи
  course_info.course_id := 1;
  course_info.course_name := 'СП';
  course_info.teacher_info.teacher_id := 1; 
  course_info.teacher_info.teacher_name := 'Смелов Владимир Владиславович';
  course_info.teacher_info.department := 'ИСиТ';
  course_info.student_info.student_id := 1;
  course_info.student_info.student_name := 'Коршун Никита';
  course_info.student_info.major := 'ИТ';

  -- Вывод значений полей типа записи
