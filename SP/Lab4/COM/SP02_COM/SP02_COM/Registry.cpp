	...
	const WCHAR* szProgID)       //   IDs
{
	...
	WCHAR szModule[512];
	DWORD dwResult = GetModuleFileName(hModule, szModule, sizeof(szModule) / sizeof(WCHAR)); //Получает путь к исполняемому файлу текущего модуля (DLL) 
	assert(dwResult != 0);

	WCHAR szCLSID[CLSID_STRING_SIZE];
	CLSIDtochar(clsid, szCLSID, sizeof(szCLSID));        //Преобразует CLSID в строку с помощью функции

	//Создает записи в реестре для CLSID, ProgID и VersionIndependentProgID, связывая их с метаданными сервера и его путем.
...

{
	...
	wcscpy_s(szKey, L"CLSID\\");
	wcscat_s(szKey, szCLSID);
	//екурсивно удаляет ключи и подключи из реестра.
